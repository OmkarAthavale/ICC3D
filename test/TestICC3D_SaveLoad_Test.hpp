...
// *************************** TEST ************************************* //
class TestICC3D_SaveLoad_Test : public CxxTest::TestSuite
{
public:
		std::set<unsigned> elementIndexesICC;
		void TestScheduler()
		{
			int tSim = 2000; //total simulation time (ms)
			int dtSave = 500; //simulation saving interval (ms)
...
private:
 ...
 // Setting Attributes
		...
		// Set outputfile name
				std::string dirName = "TestMesh3D_ICCDummyBath_dt"+std::to_string(dtWrite)+"ms_"+std::to_string(tStart)+"-"+std::to_string(tEnd)+"ms";
    ...
    {
				std::string dirSavedSim = "TestMesh3D_ICCDummyBath_dt"+std::to_string(dtWrite)+"ms_"+std::to_string(tStart-dtSave)+"-"+std::to_string(tEnd-dtSave)+"ms"+"/saved_simulation";
