...
#include "MathsCustomFunctions.hpp"
    ...
    Cellimtiaz_2002d_noTstart_CORFromCellML::Cellimtiaz_2002d_noTstart_CORFromCellML(boost::shared_ptr<AbstractIvpOdeSolver> pSolver, boost::shared_ptr<AbstractStimulusFunction> pIntracellularStimulus, ModParamHolder mod)
        : AbstractCardiacCell(
                pSolver,
                7,
                0,
                pIntracellularStimulus)
    {
        ...
        for(int i=0; i<4; i++)
        {
            SetParameter(pMod[i]->GetName(), pMod[i]->GetValue());
        }
        SetParameter("cor", 1.0);
    }
    ...
    const ModParamHolder Cellimtiaz_2002d_noTstart_CORFromCellML::GetModifiableParams() const
    {
        return pMod;
    }
    ...
    double Cellimtiaz_2002d_noTstart_CORFromCellML::GetIIonic(const std::vector<double>* pStateVariables)
    {
        ...
        const double var_i_BK__G_max_BK = this->GetParameter("inhibitory_neural"); // conductance_units
        ...
        const double var_i_BK__E_K =  this->GetParameter("E_K"); // voltage_units
    ...
    void Cellimtiaz_2002d_noTstart_CORFromCellML::EvaluateYDerivatives(double var_chaste_interface__Time__time, const std::vector<double>& rY, std::vector<double>& rDY)
    {
        ...
        for(int i=0; i<4; i++)
        {
            SetParameter(pMod[i]->GetName(), pMod[i]->GetValue(var_chaste_interface__Time__time));
        }
        ...
        const double var_i_Na__V_m = var_Membrane__V_m; // voltage_units
        const double var_Membrane__Ca_c = var_chaste_interface__intracellular_Ca__Ca_c; // millimolar
        const double var_Membrane__Cor = this->GetParameter("cor"); // dimensionless
        ...
        const double var_intracellular_Ca__beta = this->GetParameter("excitatory_neural"); // millimolar_per_time_units
...
void OdeSystemInformation<Cellimtiaz_2002d_noTstart_CORFromCellML>::Initialise(void)
{
    ...
    // mParameters[1]: a new parameter that should be set for each cell
    this->mParameterNames.push_back("eta");
    this->mParameterUnits.push_back("per_time_units");
    // mParameters[2]: a new parameter that should be set for each cell
    this->mParameterNames.push_back("excitatory_neural");
    this->mParameterUnits.push_back("millimolar_per_time_units");
    // mParameters[3]: a new parameter that should be set for each cell
    this->mParameterNames.push_back("inhibitory_neural");
    this->mParameterUnits.push_back("conductance_units");
    // mParameters[4]: a new parameter that should be set for each cell
    this->mParameterNames.push_back("cor");
