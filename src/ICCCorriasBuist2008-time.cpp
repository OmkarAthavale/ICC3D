...
#include "ICCCorriasBuist2008-time.hpp"
#include <cmath>
#include <cassert>
#include <memory>
...
using namespace std;
    ...
    double CellICCCorriasBuist2008::Get_ICC_Membrane__Cm()
    {
        return var_ICC_Membrane__Cm;
    }
    
    double CellICCCorriasBuist2008::Get_chaste_interface__i_ionic()
    {
        return var_chaste_interface__i_ionic;
    }
    
    CellICCCorriasBuist2008::CellICCCorriasBuist2008(boost::shared_ptr<AbstractIvpOdeSolver> pSolver, boost::shared_ptr<AbstractStimulusFunction> pIntracellularStimulus)
        : AbstractCardiacCell(
                pSolver,
                22,
                0,
                pIntracellularStimulus)
    {
        ...
        // 
        this->mpSystemInfo = OdeSystemInformation<CellICCCorriasBuist2008>::Instance();
        Init();

        this->mParameters[0] = 0.0006; // (c,PU_unit__IP3) [millimolar]
	///////////////// Added ssat335 ///////////////////
	// mParameter[1] = V_excitation
	// mParameter[2] = is_passed_threshold
	// mParameter[3] = active_duration
	// mParameter[4] = kill_time
	// mParameter[5] = inactive_duration
	// mParameter[6] = live_time

	this->mParameters[1] = -67;
	this->mParameters[2] = 0;
	this->mParameters[3] = 0;
	this->mParameters[4] = 15000;
	this->mParameters[5] = 0;
	this->mParameters[6] = 5000;
	this->mParameters[7] = 0.01;
	this->mParameters[8] = 0;
	////////////////////////////////////////////////

    }
    
    CellICCCorriasBuist2008::~CellICCCorriasBuist2008()
    {
    }
    
    void CellICCCorriasBuist2008::VerifyStateVariables()
    {
        
    }
    
    double CellICCCorriasBuist2008::GetIIonic(const std::vector<double>* pStateVariables)
    {
    ...
    void CellICCCorriasBuist2008::EvaluateYDerivatives(double var_chaste_interface__Time__time, const std::vector<double>& rY, std::vector<double>& rDY)
    {
       ...
       // mParameter[6] = live_time
        ...
        double ode_time_step = mParameters[7];

        if (( rY[0] > mParameters[1] ) || ( mParameters[5] >= mParameters[6] ))
	{
	   mParameters[2] = 100; //is_passed_threshold = TRUE
	   mParameters[8] = 100; //first_cycle = TRUE
	}

	/*	cout <<  rY[0]  << "- Voltage _in " << endl;
	cout <<    mParameters[1]  << "- excitation_voltage " << endl;	
	cout <<  mParameters[2] << "-is_passed_threshold" << endl;
	cout << mParameters[3] << "-active_duration" << endl;
	cout <<  mParameters[5] << "-inactive_duration" << endl;*/

        if ((mParameters[2] < 50) || ( mParameters[3] >= mParameters[4]))
        {
	  // cout << "reset" << endl;
	  mParameters[2] = 0; //is_passed_threshold = FALSE
	  if (mParameters[8] > 50)
	    {
	      mParameters[5] += ode_time_step; 
	    }
	  mParameters[3] = 0;

	  // Cheesy way of resetting the state variable
  	  rDY[0] = (-67 - rY[0])/ode_time_step;
...
template<>
void OdeSystemInformation<CellICCCorriasBuist2008>::Initialise(void)
{
